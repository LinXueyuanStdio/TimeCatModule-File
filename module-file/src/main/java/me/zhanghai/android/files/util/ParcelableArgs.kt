...
@MainThread
inline fun <reified Args : ParcelableArgs> Fragment.args(default: Args) =
    BundleArgsLazyWithDefault(Args::class, default) {
    arguments ?: throw IllegalStateException("Fragment $this has null arguments")
}

class BundleArgsLazyWithDefault<Args : ParcelableArgs>(
    private val argsClass: KClass<Args>,
    private val default: Args,
    ...
    override val value: Args
        get() {
            var args = cached
            if (args == null) {
                args = argumentsProducer().getArgsOrNull(argsClass)
                cached = args
            }
            if (args == null) {
                args = default
